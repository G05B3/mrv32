module core_tb;
  ...
  initial begin
    retire_count = 0;
    max_retire   = 1000;  // default
    void'($value$plusargs("MAX_RETIRE=%d", max_retire));
  end

  localparam DEFAULT_MEM_START = 32'h00001000;
  localparam DEFAULT_MEM_END   = 32'h00002000;

  logic [31:0] mem_start;
  logic [31:0] mem_end;

  initial begin
    mem_start = DEFAULT_MEM_START;
    mem_end   = DEFAULT_MEM_END;
  
    void'($value$plusargs("MEM_START=%h", mem_start));
    void'($value$plusargs("MEM_END=%h", mem_end));
  end
  ...
  always @(posedge clk) begin
    ...
    if (retire_count >= max_retire) begin
      $display("FINAL_RETIRE_COUNT=%0d", retire_count);
      dump_registers();
